...
def test_front_login(username, passwd):
    """前端登录"""
    test_front_login_url = url.host_test_front_login
    ...
    r = requests.post(url=test_front_login_url, data=data_json, headers=headers, verify=False).json()
    print(r)
    return r["entities"][0]["xAuthToken"]


def test_app_login(username, passwd):
    """APP登录"""
    test_front_login_url = url.host_test_app_login
    app_login_param = {
        "appversion": "3.8.5",
...
# test_app_login("16866066110","che001")


def submit_token():
    """青盈开标：申请借款获取submitToken值接口"""
...
def open_qy(username, amount, password):
    ...
    headers = {
        ...
        "X-Auth-Token": test_front_login(username, password)
    }
